...
| `evals_get_record` | Read a draft or submitted record by id. The verification subagent's entry point. | `id` | `readOnlyHint: true`, `openWorldHint: false` |
...
- **Grader DSL is a typed union** serialized with the record. Deterministic kinds are executed by the server itself (the agent can't merely *assert* the gold is correct); `llm_rubric` runs via `ctx.sample` when the client supports sampling, else is agent-attested and flagged `server_verified: false`.
